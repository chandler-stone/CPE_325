...
void DMA_setup(void) {
    ...
    DMA0SA = (__SFR_FARPTR)pause;   // Source block address
    DMA0DA = (__SFR_FARPTR)&UCA0TXBUF;// Destination single address
//    DMA0SZ = sizeof(timeMsg);         // Length of the String
    DMA0CTL = DMASRCINCR_0 + DMASWDW + DMALEVEL; // Increment src address
//    DMA0CTL = DMADT0 + DMADSTINCR_0 + DMASRCINCR0 + DMASWDW;
}
